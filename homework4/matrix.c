...
void SpMV(int **sparse_mat, int *vec, int *res_vec, int rows, int cols) {
    assert(rows == cols);

    for (int i = 0; i < rows; i++){
        for (int j = 0; j < cols; j++){
            res_vec[i] += sparse_mat[i][j] * vec[j];
        }
    }
}
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        SpMV(mat_trans, src, dst, rows, cols);
        for (int i = 0; i < rows; i++){
            if(src[i]) { //src color black if neighbor
                color[i] = 2;
            }
        }
        //store distance for new neighbors
        for (int i = 0; i < rows; i++){
            if ((dst[i] > 0) && color[i] == 0){
                distance[i] = iter;
                done = 0; //check for neighbors
            }
        }
        for (int i = 0; i < rows; i++){
            if (color[i] == 2){
                reset_vector(mat_trans[i], rows);

            }
        }

        reset_vector(src, rows);
        int *tmp = src;
        src = dst;
        dst =tmp;
        iter++;
