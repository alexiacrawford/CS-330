...
namespace Mem {
  ...
  public:
    ...
    friend std::ostream & generator<<(std::ostream &s, IntQueue &q);

    IntQueue & operator<<(int inData) { Enqueue(inData); return *this; }
    IntQueue & operator>>(int &outData) { outData = Dequeue(); return *this; }
...
//PART 1
std::ostream &
:operator<<(std::ostream &s, Mem::IntQueue &q) {
  Elem<int> *tmp = q.head;
  while (tmp != NULL){
    s << tmp;
    tmp = tmp->getNext();
