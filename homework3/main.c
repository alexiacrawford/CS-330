                        ...
                        double** csr_vals)
{
    ...
    *csr_vals = (double*) malloc(nnz * sizeof(double));
    ...
    if (!(*csr_row_ptr) || !(*csr_col_ind) || !(*csr_vals)) {
        fprintf(stderr, "Memory allocation failed in convert_coo_to_csr.\n"); //debugging but also good for checking
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < nnz; i++) {
        int row = row_ind[i] - 1; //adjust from 1-indexed to 0-indexed
        (*csr_row_ptr)[row + 1]++; //count nonzero for row
    }

    //sum to get the last row pointer array.
    for (int i = 0; i < m; i++) {
        (*csr_row_ptr)[i + 1] += (*csr_row_ptr)[i];
    }

    //create a temp copy of csr_row_ptr to use as insertion pointers
    unsigned int* temp = (unsigned int*) malloc((m + 1) * sizeof(unsigned int));
    if (!temp) {
        fprintf(stderr, "Memory allocation failed in convert_coo_to_csr (temp array).\n");
        exit(EXIT_FAILURE);
    }
    memcpy(temp, *csr_row_ptr, (m + 1) * sizeof(unsigned int));

    //fill csr_col_ind and csr_vals with coo data.
    for (int i = 0; i < nnz; i++) {
        int row = row_ind[i] - 1;  // adjust row index
        unsigned int pos = temp[row];
        (*csr_col_ind)[pos] = (unsigned int)(col_ind[i] - 1); //adjust column indices from 1-indexed to 0-indexed
        (*csr_vals)[pos] = val[i];
        temp[row]++;
    }
    free(temp);
